...
class Libraries {
	...
	public static function init() {
		...
		self::$mono_libraries['parsedown'] = [
			'name'    => 'Parsedown',
			'detect'  => 'Parsedown',
			'base'    => OEMM_VENDOR_DIR . 'parsedown/',
			'version' => '1.8.0-beta-7',
			// phpcs:ignore
			'author'  => sprintf( esc_html__( '%s & contributors', 'oembed-manager' ), 'Emanuil Rusev' ),
	...
	private function license_name( $license ) {
		switch ( $license ) {
			case 'mit':
				$result = esc_html__( 'MIT license', 'oembed-manager' );
				break;
			case 'apl2':
				$result = esc_html__( 'Apache license, version 2.0', 'oembed-manager' );
				break;
			case 'gpl2':
				$result = esc_html__( 'GPL-2.0 license', 'oembed-manager' );
				break;
			case 'gpl3':
				$result = esc_html__( 'GPL-3.0 license', 'oembed-manager' );
				break;
			default:
				$result = esc_html__( 'unknown license', 'oembed-manager' );
				break;
		}
		return $result;
	}
	...
	public function sc_get_list( $attributes ) {
		...
		$item['name']    = 'Switchery';
		$item['version'] = '0.8.2';
		$item['author']  = sprintf( esc_html__( '%s & contributors', 'oembed-manager' ), 'Alexander Petkov' );
		...
		$item['version'] = '3.0.5';
		$item['author']  = sprintf( esc_html__( '%s & contributors', 'oembed-manager' ), 'Dan Grossman' );
		...
		$item['name']    = 'Moment';
		$item['version'] = '2.24.0';
		$item['author']  = sprintf( esc_html__( '%s & contributors', 'oembed-manager' ), 'Tim Wood' );
		...
		$item['version'] = '1.0.2';
		$item['author']  = sprintf( esc_html__( '%s & contributors', 'oembed-manager' ), 'Sam Herbert' );
		...
		$item['version'] = '0.11.4';
		$item['author']  = sprintf( esc_html__( '%s & contributors', 'oembed-manager' ), 'Gion Kunz' );
		...
		if ( 'html' === $style ) {
			...
			foreach ( $list as $library ) {
				...
				$items[] = sprintf( __( '<a href="%1$s">%2$s %3$s</a>%4$s by %5$s (%6$s)', 'oembed-manager' ), $library['url'], $library['name'], $library['version'], $library['langs'], $library['author'], $library['license'] );
			}
