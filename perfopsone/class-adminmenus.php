...
if ( ! class_exists( 'PerfOpsOne\AdminMenus' ) ) {
	class AdminMenus {
		...
		public static function initialize() {
			foreach ( apply_filters( 'init_perfops_admin_menus', [] ) as $menu => $submenus ) {
				if ( ! in_array( 'perfopsone-' . $menu, self::$slugs, true ) ) {
					switch ( $menu ) {
						case 'analytics':
							add_menu_page( esc_html__( 'Dashboard', 'oembed-manager' ), sprintf( esc_html__( '%s Analytics', 'oembed-manager' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_analytics_page' ], 'dashicons-chart-bar', 210874 );
							...
							break;
						case 'tools':
							add_menu_page( esc_html__( 'Available Tools', 'oembed-manager' ), sprintf( esc_html__( '%s Tools', 'oembed-manager' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_tools_page' ], 'dashicons-admin-tools', 210876 );
							...
							break;
						case 'insights':
							add_menu_page( esc_html__( 'Available Reports', 'oembed-manager' ), sprintf( esc_html__( '%s Insights', 'oembed-manager' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_insights_page' ], 'dashicons-lightbulb', 210875 );
							...
							break;
						case 'records':
							add_menu_page( esc_html__( 'Available Catalogues', 'oembed-manager' ), sprintf( esc_html__( '%s Records', 'oembed-manager' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_records_page' ], 'dashicons-book', 210873 );
							...
							break;
						case 'settings':
							add_menu_page( esc_html__( 'Control Center', 'oembed-manager' ), sprintf( esc_html__( '%s Settings', 'oembed-manager' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_settings_page' ], 'dashicons-admin-settings', 210871 );
							...
							break;
						case 'consoles':
							add_menu_page( esc_html__( 'Available Consoles', 'oembed-manager' ), sprintf( esc_html__( '%s Consoles', 'oembed-manager' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_consoles_page' ], 'dashicons-embed-generic', 210872 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Available Consoles', 'oembed-manager' ), __( 'Available Consoles', 'oembed-manager' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_consoles_page' ], 0 );
							break;
					}
		...
		private static function display_as_lines( $items ) {
			...
			foreach ( $items as $item ) {
				$links   = [];
				$links[] = '<a href="' . $item['url'] . '"/>' . __( 'settings', 'oembed-manager' ) . '</a>';
				$links[] = '<a href="' . $item['url'] . '&tab=about"/>' . __( 'about', 'oembed-manager' ) . '</a>';
				$links[] = '<a href="https://wordpress.org/support/plugin/' . $item['slug'] . '" target="_blank">' . __( 'support', 'oembed-manager' ) . '</a>';
				$links[] = '<a href="https://github.com/Pierre-Lannoy/wp-' . $item['slug'] . '" target="_blank">' . __( 'contribution', 'oembed-manager' ) . '</a>';
				$disp   .= '<div class="po-container">';
				$disp   .= ' <div class="po-actionable">';
